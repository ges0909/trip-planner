...
Place immediately after the day heading:

```markdown
[📍 Google Maps Route](https://www.google.com/maps/dir/{lat1},{lon1}/{lat2},{lon2}/...)

<summary>🗺️ Statische Karte</summary>
...
- **In Web / Chat mode:** Do NOT insert broken `![...](maps/...)` image links when PNG files do not exist on disk. Interactive mapping is provided in the web application.
- **Google Maps link:** Always provide `[📍 Google Maps Route](...)` with unambiguous coordinate pairs from the routed waypoints (`lat,lon`).
...
- Include hiking options where they fit the route, destination, and user interests; do not force a hiking option on every day.
