...
class TestTimeoutHandling:
    ...
    async def test_timeout_propagates_from_wait_for(
        ...
        def _raise_timeout(coro, timeout):
            # asyncio.wait_for is mocked out entirely, so the readline()
            # coroutine passed in is never awaited by the real wait_for.
            # Close it explicitly to avoid an unawaited-coroutine warning.
            coro.close()
            raise TimeoutError()

        # Patch asyncio.wait_for to simulate the 60s timeout expiring
        with (
            patch("core.mcp_manager.asyncio.wait_for", side_effect=_raise_timeout),
