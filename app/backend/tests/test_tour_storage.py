...
class TestListTrash:
    ...
    def test_list_trash_with_items(self, mock_paths):
        ...
        _trips_dir, trash_dir = mock_paths
        import storage.tour_storage as tour_storage
...
class TestValidation:
    """Tests for slug, trash_name, and map filename validation."""

    def test_is_valid_slug(self):
        from storage.tour_storage import is_valid_slug

        assert is_valid_slug("berlin-potsdam-radtour") is True
        assert is_valid_slug("alpen-cross-2024") is True
        assert is_valid_slug("route-1") is True
        assert is_valid_slug("a") is True

        assert is_valid_slug("") is False
        assert is_valid_slug("../dangerous") is False
        assert is_valid_slug("invalid/slash") is False
        assert is_valid_slug("invalid\\backslash") is False
        assert is_valid_slug(".hidden") is False
        assert is_valid_slug("uppercase_NOT_ALLOWED") is False
        assert is_valid_slug("-leading-hyphen") is False
        assert is_valid_slug("trailing-hyphen-") is False
        assert is_valid_slug("x" * 101) is False

    def test_is_valid_trash_name(self):
        from storage.tour_storage import is_valid_trash_name

        assert is_valid_trash_name("my-tour_20260830-114500") is True
        assert is_valid_trash_name("my-tour") is True

        assert is_valid_trash_name("") is False
        assert is_valid_trash_name("../path_traversal") is False
        assert is_valid_trash_name("invalid/name") is False
        assert is_valid_trash_name("x" * 151) is False

    def test_is_valid_map_filename(self):
        from storage.tour_storage import is_valid_map_filename

        assert is_valid_map_filename("overview.png") is True
        assert is_valid_map_filename("stage-1_elevation.png") is True

        assert is_valid_map_filename("route.jpg") is False
        assert is_valid_map_filename("../route.png") is False
        assert is_valid_map_filename("route.png.exe") is False
        assert is_valid_map_filename("") is False

    def test_get_tour_path_raises_on_invalid_slug(self):
        from storage.tour_storage import get_tour_path

        with pytest.raises(ValueError, match="Invalid tour_type"):
            get_tour_path("invalid_type", "my-tour")

        with pytest.raises(ValueError, match="Invalid tour_type"):
            get_tour_path("bike", "../escape")
